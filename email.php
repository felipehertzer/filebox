...
<center>
        ...
        if (isset($_POST['sendEmail'])) {
            ...
            if (!empty($select1)) {
             ...
             <p style=\"font-family:\"arial\",sans-serif;padding:0 0 0 10px;font-size:13px;color:#333333;\"><a href=\"{$website}\"><b>{$webtitle}</b></a> - File Hosting &amp; Sharing Network</p>";
                ...
                $err = "<div class='infobox'><p>Your mail has been sent.</p></div>";
                emailForm();
            } else {
                $err = "<div class='errorbox'><p>You must fill in all the fields.</p></div>";
                emailForm();
            }
        } else {
            emailForm();
    ...
    function emailForm()
    {
        ...
        if (mysqli_num_rows($q)) {
            ...
            if ($is_folder == 1) {
                $src = "{$website}/images/labels/folder.png";
                $type = "Folder";
            } else {
                $ext = $f['extension'];
                if (($ext == "gif") || ($ext == "jpg") || ($ext == "jpeg") ||
                    ($ext == "png") || ($ext == "bmp") || ($ext == "pjpeg")) {
                    $src = "{$website}/small.php?id={$id}";
                    $type = "Photo";
                } else {
                    $src = "{$website}/showicon.php?id={$ext}";
                    $type = "File";
                }
            }
            ?>
            <div class='emaildiv' id='email'>
                <div id='emaildiv-header'><p><img src='<?php echo $src; ?>'/> Share <?php echo $type; ?> <small>(Share
                            this <?php echo $type; ?> with all those who matter)</small></p></div>
                <?php echo $err; ?>
                <form action="<?php echo $website; ?>/email.php?id=<?php echo $id; ?>" method="POST"
                    ...
                    <table id='messagecompose'>
                        ...
                        <tr>
                            <td></td>
                            <td><input type='hidden' name='folder' value='<?php echo $is_folder; ?>'/><input
                                        type='submit' class='button' value='Send' name='sendEmail'/></td>
                        </tr>
                    </table>
                </form>
