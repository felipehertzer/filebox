<?php
 ...
 <?php if(isset($_POST['multicode'])) {
	   ...
	   $counter = mysqli_num_rows($query_im);
			...
			$x = 0; echo "<table id='mainimages'><tr>";
			...
			$x++; if(($x % 3) == 0) echo "</tr><tr>";
			}
			echo "</tr></table>";
	   } else { echo "<div class='errorbox'><p>No files uploaded to the folder.</p></div>"; }			
			} else {
			if($counter) { 
			while($fetch_im = mysqli_fetch_array($query_im)) {
			...
			echo "<img src='{$website}/small.php?id={$fetch_im['id']}' />"; } else { echo "<img src='{$website}/showicon.php?id={$fetch_im['extension']}' />";
			}
	   ...
	   } else { echo "<div class='errorbox'><p>You must upload atleast one file.</p></div>"; }
	 }
	 echo "</div>";
	 }
	 ...
	 $max = 10; $total = 0;
     $date = date("d M Y");$ip = $_SERVER['REMOTE_ADDR']; $zY = date("zY");
	 $ffolder = "uploads/files/".$zY; $pfolder = "uploads/photos/".$zY; $sfolder = "uploads/photos/".$zY."/small";
	 $Time = date('YmdHis');
	 ...
	 if(!is_dir($ffolder)) { mkdir("uploads/files/{$zY}", 0755); }
	 if(!is_dir($pfolder)) { mkdir("uploads/photos/{$zY}", 0755); }
	 if(!is_dir($sfolder)) { mkdir("uploads/photos/{$zY}/small", 0755); }
	 ...
	 if($secfile != "") { $total = $total + 1; $name = "file".$i;
	 ...
	 $rand_1 = rand(1000, 100000); $rand_2 = rand(100000, 100000000); $rand_3 = rand();
     ...
     if($handle->processed)
     {
		...
		echo "</td></tr></table>";
	 } else { echo "<div class='errorbox'><p>Not a valid format<br/><small><b>.{$extension}</b> files are not allowed to be uploaded on {$webtitle}</small></p></div>";
	 }	
	 ...
	 } else { echo "<div class='errorbox'><p>An error occured<br/><small>" . $handle->error . "</small></p></div>"; }
	 } else { echo "<div class='errorbox'><p>File too big<br/><small>" . $handle->error . "</small></p></div>"; }
	 } else { echo "<div class='errorbox'><p>Select a file to upload</p></div>"; }
	 } // END $SECFILE PROCESSING
	 } 
	
	// IF NO. OF FILES UPLOADED IS ZERO THEN PERFORM THE FOLLOWING
	if($total == 0) { if(isset($_GET['fid'])) { echo "<div class='errorbox'><p>No files uploaded to the folder.</p></div>"; } else { echo "<div class='errorbox'><p>You must upload atleast one file.</p></div>"; } }
	if(isset($_GET['fid'])) { $foldername = htmlspecialchars(trim($_POST['foldername']));
	...
	} else { echo "<div id='main'><div id='account-header'><p>Upload Details</p></div><div class='errorbox'><p>Please use the upload page for this action.</p></div></div>"; } ?>
