...
$id = preg_replace('/\D/', '', trim($_GET['id']));

if (isset($id)) {
    $stmt = mysqli_prepare($link, "SELECT `name`, `location`, `extension` FROM `files` WHERE(`id`= ?)") or die(mysqli_error($link));
    ...
    if (mysqli_num_rows($q)) {
        ...
        $title = $f['name'];
        $tmp_extension = $f['extension'];
        $tmp_location = $f['location'];
    } else {
...
$css = "<link type='text/css' rel='stylesheet' href='{$website}/css/colorbox.css' />";
    ...
    <script>
        <?php if ($sesslife == true) {
            if ($premium == 1) {
                ...
                echo "var limit=\"{$normal_timer}\";";
            }
        } else {
            echo "var limit=\"{$anon_timer}\";";
        }
        ?>
        if (document.images) {
...
if (isset($id)) {
    $stmt = mysqli_prepare($link, "SELECT * FROM `files` WHERE(`id`= ?)") or die(mysqli_error($link));
    mysqli_stmt_bind_param($stmt, "i", $id);
    $result = mysqli_stmt_execute($stmt);
    $d_query = mysqli_stmt_get_result($stmt);

    if (mysqli_num_rows($d_query)) {
        $f_query = mysqli_fetch_array($d_query);
            ...
            <div id='main'>
                <div id='downleft'>
                    <div id='downext'>
                        ...
                        if (($ext == "gif") || ($ext == "jpg") || ($ext == "jpeg") ||
                            ...
                            if (!file_exists($f_query['location'])) {
                                $f_query['location'] = "images/file_error.png";
                            }
                            echo "<img src='{$website}/{$f_query['location']}' width='{$mainWidth}' height='{$mainHeight}' />";

                        } else {
                            echo "<div id='mp3player'></div><img src='{$website}/showicon.php?id={$ext}&big=1' width='250px' height='250px' />";
                        } ?>
                ...
                <div id='downright'>
                    ...
                    <div class='suboptions'>
                        <ul>
                            <li id='multi'><a href='<?php echo $website; ?>/user/upload.php'>Upload</a></li>
                            <?php if (($ext == "gif") || ($ext == "jpg") || ($ext == "jpeg") ||
                                ($ext == "png") || ($ext == "bmp") || ($ext == "pjpeg")) { ?>
                                <li id='bigger'><a href='<?php echo $website . "/" . $f_query['location']; ?>'
                                                   target='_blank'>View Big</a></li>
                            <?php } ?>
                            <li id='download'><a href='#' onClick='begintimer();'>Download</a></li>
                            <li id='email'><a href='<?php echo $website; ?>/email.php?id=<?php echo $id; ?>'
                                              id='emailFile'>Email File</a></li>
                            <?php if ($ext == "flv") { ?>
                                <li id='play'><a href='<?php echo $website; ?>/play.php?id=<?php echo $id; ?>'
                                                 id='playmovie'>Play</a></li>
                            <?php } ?>
                            <li id='stats'><a href='#' id='fileStats'
                                              href='<?php echo $website; ?>/filestatistics.php?id=<?php echo $id; ?>'>Statistics</a>
                            </li>
                    </div>

                    <?php $views = $f_query['views'] + 1;
                    $views_update = mysqli_query($link, "UPDATE `files` SET `views`={$views} WHERE `id`={$id}") or die(mysqli_error($link)); ?>
                    ...
                    <div id='filedetails'>
                        ...
                        <table>
                            ...
                            <tr>
                                ...
                                <td><?php echo $f_query['size']; ?> KB</td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                ...
                                <td><?php echo $f_query['downloads']; ?></td>
                            </tr>
                            <tr>
                                <td class='first'>Uploaded</td>
                                <td><?php echo $f_query['date']; ?></td>
                            </tr>
                        </table>

                        <div id='downloadfile'><input type='button' name='downloadbtn' value='Download File'
                                                      class='button' id='downloadbtn' onClick='begintimer();'/>
                            <center><input type='text' name='secondtxt' value='' id='secondtxt' disabled='disabled'/>
                            </center>
                        </div>
                    </div>
