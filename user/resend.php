<?php
  ...
  if(isset($_POST['resend'])) {
    ...
    if ($resp->is_valid) {
		...
		if($verified == 0) {
				...
				else { newuser_email($email, $key);
				$err = "<div class='infobox'><p>{$lang[83]}</p></div>"; am_showResend();
				}	
		} else { $err = "<div class='infobox'><p>{$lang[84]}</p></div>"; am_showResend(); }
		} else { $err = "<div class='errorbox'><p>{$lang[53]}</p></div>"; am_showResend(); }		
		} else { $err = "<div class='errorbox'><p>{$lang[54]}</p></div>"; am_showResend(); }	
	} else { $err = "<div class='errorbox'><p>{$lang[55]}</p></div>"; am_showResend(); }
  }
  else { am_showResend(); }
  } else { echo "<br/><center><div class='errorbox'><p>{$lang[40]}</p></div></center><br/><br/>"; }
   
  function am_showResend() { global $lang; global $website; global $publickey; global $webtitle; global $err; ?>
  <center><form method='POST' action='<?php echo $website; ?>/user/resend.php'>
  <div class='logindiv'>
  ...
  </td></tr><tr><td></td><td id='join-now'><input type='submit' name='resend' class='button' value='<?php echo $lang[28]; ?>' /></td></tr>
  <tr><td></td><td><small><?php echo $lang[10]; ?> <?php echo $webtitle; ?>? <a href='<?php echo $website; ?>/user/login.php'><?php echo $lang[105]; ?></a></small></td></tr> 
