<?php
 ...
 $rand_1 = rand(1000, 100000); $rand_2 = rand(100000, 100000000); $rand_3 = rand(); $rand_4 = md5(microtime().rand(0,999999));
 $zipName = "../uploads/zip/";
 $zipName .= "random_".$rand_1."_".$rand_2."_".$rand_3."_".$rand_4.".zip";
 $createZipFile = new CreateZipFile;
 $ip = $_SERVER['REMOTE_ADDR']; $date_bd = date("d M Y");
 ...
 <script type='text/javascript'>
		 ...
		 } else { document.getElementById(tr).style.background = "#fdfdfd"; }
		 }
		 }}
 </script>
 
<?php if($sesslife == true) { $currentUrl = current_url();
 
 if(empty($_SESSION['order'])) { $_SESSION['order'] = "ORDER BY `id`"; }
 if(isset($_GET['o'])) { $o = htmlspecialchars(trim($_GET['o']));
	switch($o) {
 ...
 if(isset($_GET["p"])) { $page = htmlspecialchars(trim($_GET["p"])); }   
 else { $page = 1; } $max_show = 10;
 
 $sql = mysqli_query("SELECT * FROM `files` WHERE(`userid`={$userid}) AND (`parent`=0) {$_SESSION['order']} DESC") or die(mysqli_error());
 $sql_n = mysqli_num_rows($sql);
 
 if($sql_n != 0) {
 
 $p = new pagination;$p->items($sql_n);
 $p->limit(10);$p->currentPage($page);$p->parameterName("p");
 ...
 $unique_id[] = $sql_f['id']; $extension[] = $sql_f['extension']; $date[] = $sql_f['date']; $filter[] = $sql_f['filter'];
 $downloads[] = $sql_f['downloads']; $size[] = $sql_f['size']; $fname[] = $sql_f['name']; $is_folder[] = $sql_f['is_folder'];
 $views[] = $sql_f['views'];
 }
 ...
 echo "<form name='files' method='POST' action='{$currentUrl}'>"; ?>
 	...
 	<div id='sort'><ul><li id='input'><input type='checkbox' name='checkall' onclick='checkUncheckAll(this);' /></li>
	<li>Sort By:</li><a href='?o=id'><li<?php if($_SESSION['order'] == "ORDER BY `id`") { echo " class='selected'"; } ?>>Recent</li></a>
	<a href='?o=name'><li<?php if($_SESSION['order'] == "ORDER BY `name`") { echo " class='selected'"; } ?>>Name</li></a>
	<a href='?o=date'><li<?php if($_SESSION['order'] == "ORDER BY `date`") { echo " class='selected'"; } ?>>Date</li></a>
	<a href='?o=size'><li<?php if($_SESSION['order'] == "ORDER BY `size`") { echo " class='selected'"; } ?>>Size</li></a>
	<a href='?o=downloads'><li<?php if($_SESSION['order'] == "ORDER BY `downloads`") { echo " class='selected'"; } ?>>Downloads</li></a>
	</ul></div>
...
<?php echo "<div id='ajax'></div>";
 ...
 $fid = $unique_id[$i]; $ext = $extension[$i]; $d = $date[$i]; $downlds = $downloads[$i];
 $s = $size[$i]; $file_name = $fname[$i]; $ff = $filter[$i]; $is_f = $is_folder[$i]; $v = $views[$i];
 
 if($ff == 0) { $already = "Public"; $make = "Private"; $class_red = "plain"; $change_filter = 1; }
 else { $already = "Private"; $make = "Public"; $class_red = "red"; $change_filter = 0; }
	
	if($is_f == 1) {
	$fc_query = mysqli_query("SELECT count(*) AS count FROM `files` WHERE(`userid`={$userid}) AND (`is_folder`=1) AND (`parent`={$fid})");
	$folder_count = mysqli_result($fc_query, 0);
	
	$filec_query = mysqli_query("SELECT count(*) AS count FROM `files` WHERE(`userid`={$userid}) AND (`is_folder`!=1) AND (`parent`={$fid})");
	...
	echo "<img src='{$website}/small.php?id={$fid}' />"; } else {
 ...
 } else { $err = "<div class='infobox'><p>Login to continue<br/><small>You does not seem to be logged in to the website. Please login in order to view this page.</small></p></div>"; 
 am_showLogin(); }
