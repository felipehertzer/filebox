<?php
 ...
 $r = htmlspecialchars(trim($_GET['r']));
	...
	$q = "SELECT * FROM `members` WHERE (email = '{$username}') and (password = '{$password}')";
        ...
        if(!$n2) { $err =  "<div class='errorbox'><p>{$lang[37]}</p></div>"; showcaptcha($username); }
		else { $f = mysqli_fetch_array($result_set);
             ...
             showcaptcha($username);
				...
				if($banned == 1) { $err =  "<div class='errorbox'><p>{$lang[48]}<br/><small>{$lang[57]}</small></p></div>"; 
             showcaptcha($username); } else {
             $date = date("d M Y");
             $q = mysqli_query("UPDATE `members` SET access = '{$date}' WHERE email = '{$username}'");
			 $up = mysqli_query("UPDATE `members` SET `login_attempt` = 0 WHERE `email`='{$username}' LIMIT 1") or die(mysqli_error());
	...
	else { $err = "<div class='errorbox'><p>{$lang[54]}</p></div>"; showcaptcha($username);}
	}
	else { $err = "<div class='errorbox'><p>{$lang[39]}</p></div>"; showcaptcha($username);}
   }
  } 
 ...
 if(isset($_POST["login"]))  {
    ...
    if(($username != "") && ($password != ""))
    {
		$a = mysqli_query("SELECT login_attempt FROM `members` WHERE(email='{$username}')") or die(mysqli_error());
		$ac = mysqli_num_rows($a);
		if($ac) { $f = mysqli_fetch_array($a); $login_attempt = $f['login_attempt'];
		if($login_attempt > 4) { $err = "<div class='errorbox'><p>{$lang[73]}</p></div>"; showcaptcha($username); }
		else {
        $q = "SELECT * FROM `members` WHERE (email = '{$username}') and (password = '{$password}')";
        if(!($result_set = mysqli_query($q))) die(mysqli_error());
        $n2 = mysqli_num_rows($result_set);
        ...
        if (!$n2) {
			...
			$up = mysqli_query("UPDATE `members` SET `login_attempt` = '{$attempt}' WHERE `email`='{$username}' LIMIT 1") or die(mysqli_error());
            ...
            am_showLogin();
             ...
             am_showLogin(); }
				...
				if($banned == 1) { $err =  "<div class='errorbox'><p>{$lang[48]}<br/><br/><small>{$lang[57]}</small></p></div>"; 
             am_showLogin(); } else {
             $date = date("d M Y");
             $q = mysqli_query("UPDATE `members` SET access = '{$date}' WHERE email = '{$username}'");
			 $up = mysqli_query("UPDATE `members` SET `login_attempt` = 0 WHERE `email`='{$username}' LIMIT 1") or die(mysqli_error());
		...
		} } } else { $err =  "<div class='errorbox'><p>{$lang[37]}</p></div>"; am_showLogin(); }
		}
   else
     { $err =  "<div class='errorbox'><p>{$lang[39]}</p></div>"; am_showLogin(); }
}
 }
else
{
