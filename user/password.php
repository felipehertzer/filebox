...
if ($sesslife == true) {
            ...
            if ($currentpass == $userpass) {
                if ($newpass == $confirmpass) {
                    if (strlen($newpass) <= 25) {
                        ...
                        $err = "<div class='infobox'><p>Password changed<br/><small>Your password has been changed successfully and is effective immediately.</small></p></div>";
                        ft_changepass();
                    } else {
                        $err = "<div class='errorbox'><p>Password too long<br/><small>Your password contains more than 25 characters. Please restrict your password to 25 or less characters.</small></p></div>";
                        ft_changepass();
                    }
                } else {
                    $err = "<div class='errorbox'><p>Password mismatch<br/><small>The new password and confirm password entered by you do not match. Make sure they both are same. This is used to double check the password entered by you.</p></div>";
                    ft_changepass();
                }
            } else {
                $err = "<div class='errorbox'><p>Current password is not correct<br/><small>The current password entered by you does not match with the one we have in our records. Please recheck and enter correct password.</small></p></div>";
                ft_changepass();
            }
        } else {
            $err = "<div class='errorbox'><p>Please fill in all the fields</p></div>";
            ft_changepass();
        }
    } else {
        ft_changepass();
    ...
    $err = "<div class='infobox'><p>Not Logged Version<br/><small>You does not seem to be logged in to the website. Please login in order to view this page.</small></p></div>";
    am_showLogin();
}

include("footer.php");

function ft_changepass()
{
    global $err;
