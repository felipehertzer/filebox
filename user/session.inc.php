<?php
   ...
   $sesslife = false; $userid = 0; $premium = 0;
   /* This page checks and validates whether the current session user is logged in to
   the website or not */
   
	function createRandomPassword() { /* For generating random passwords */
   ...
   if($cookie)
   {
	  ...
	  $fb_query = mysqli_query("SELECT * FROM `members` WHERE(`email`='{$fb_email}')") or die(mysqli_error());
		...
		if(mysqli_num_rows($fb_query)) {
			...
			$fb_update = mysqli_query("UPDATE `members` SET `access`='{$access_date}', `fb_id`={$temp_fb_id} WHERE(`id`={$userid})") or die(mysqli_error());
		} else {
			...
			$fb_insert = mysqli_query("INSERT INTO `members`(`name`, `password`, `email`, `key`, `join`, `access`, `fb_id`) VALUES('{$username}', '{$temp_password}', '{$fb_email}', '{$temp_key}', '{$access_date}', '{$access_date}', {$temp_fb_id})") or die(mysqli_error());
   ...
   elseif(isset($_SESSION["user"]))
   {
      ...
      if(!$n1) { $session->stop();$sesslife = false; $userid = 0;}
      else { $r = mysqli_fetch_array($result_set);
