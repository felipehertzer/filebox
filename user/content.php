...
if (isset($_GET['v'])) {
    ...
    if ($sesslife == true) {
        ...
        if ($v == 'create') {
            ...
            if (mysqli_num_rows($sql_fq)) {
                echo "<td><select id='folders' name='folders'>";
                echo "<option value='0'>My Files</option>";
                while ($sql_fetch = mysqli_fetch_array($sql_fq)) {
                    echo "<option value='{$sql_fetch['id']}'>{$sql_fetch['name']}</option>";
                }
                echo "</select></td>";
            } else {
                echo "<td><select id='folders' name='folders'><option value='0'>My Files</option></select></td>";
            }
	...
	<p class='impInfo'><span class='head'>Bandwidth Used Today</span>: ";
            user_bandwidth($userid);
            echo " MB</p>
	<p class='impInfo'><span class='head'>Total Bandwidth Consumption</span>: ";
            total_bandwidth($userid);
            echo " MB <small>(Approx.)</small></p>
	<p class='impInfo'><span class='head'>File Downloads</span>: ";
            file_downloads($userid);
