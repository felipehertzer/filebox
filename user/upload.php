...
include("../init.php");
$upload_q = mysqli_query($link, "SELECT `id` FROM `uploads` ORDER BY `id` DESC LIMIT 1") or die(mysqli_error($link));
...
if ($sesslife == true) {
    ...
    if ($percent < 100) {
        ...
        if ($v == "multiupload") {
            ...
            <div class='errorbox'><p>The all new <b>Multi Uploader</b>. Select all files at once and upload all of them
                    with a single click.</p></div>
            <table id='uploader'>
                <tr>
                    <td style='vertical-align:top;'>
                        <div id="flash_uploader"><p>Your browser does not seem to have Flash installed. Please install
                                Flash Player for your browser to use <b>Multi Uploader</b>.</p></div>
                    </td>
                </tr>
            </table>
            ...
            <div class='sidebar'>
                ...
                <a href='<?php echo $website; ?>/user/upload.php?v=multiupload'><p id='multi' class='selected'>Multi
                        Upload</p></a>
            </div>
