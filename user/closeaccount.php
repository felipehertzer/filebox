<?php
 ...
 if($sesslife == true) {
	echo "<center><div class='logindiv'><div id='logindiv-header'><p>Account Closure - Final Step</p></div><br/>";
		if(isset($_POST['confirmdelete'])) {
			$sql = mysqli_query("DELETE FROM `files` WHERE(`userid`={$userid})") or die(mysqli_error());
			$sql_2 = mysqli_query("DELETE FROM `members` WHERE(`id`={$userid})") or die(mysqli_error());
			$sql_3 = mysqli_query("DELETE FROM `transactions` WHERE(`userid`={$userid})") or die(mysqli_error());
			$session->stop(); echo "<meta http-equiv='Refresh' Content='1;URL={$website}/' />";
 ...
 echo "<br/></div></center>";
 } else { $err = "<div class='infobox'>Not Logged Version<br/><small>You does not seem to be logged in to the website. Please login in order to view this page.</small></div>";
 ft_showLogin(); }
